...
if(head=='false'){
    if(updateBalance>0){
        const balanceUpdate=await userModel.findByIdAndUpdate(userid,{ $set: { balance: updateBalance } },
            { new: true });
        
        req.body.amount=product.price
        const orderData=req.body
        const orderFinal=await orderModel.create(orderData)
        res.send({msg: "order is created successfully",orderDetails:orderFinal})
...
const orderData=req.body
const orderFinal=await orderModel.create(orderData)
const populated=await orderModel.find().populate("UserMid")
